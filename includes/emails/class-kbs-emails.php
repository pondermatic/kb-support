 ...
 * Emails
 *
 * This class handles all emails sent through KBS
 *
...
class KBS_Emails {
	...
	public function __construct() {
		...
		add_action( 'kbs_email_send_before', array( $this, 'send_before' ) );
		add_action( 'kbs_email_send_after', array( $this, 'send_after' ) );
	...
	public function get_from_name() {
		if ( ! $this->from_name ) {
			$this->from_name = kbs_get_option( 'from_name', get_bloginfo( 'name' ) );
		}

		return apply_filters( 'kbs_email_from_name', wp_specialchars_decode( $this->from_name ), $this );
	} // get_from_name
	...
	public function get_from_address() {
		if ( ! $this->from_address ) {
			$this->from_address = kbs_get_option( 'from_email', get_option( 'admin_email' ) );
		}

		return apply_filters( 'kbs_email_from_address', $this->from_address, $this );
	} // get_from_address
	...
	public function get_content_type() {
		if ( ! $this->content_type && $this->html ) {
			$this->content_type = apply_filters( 'kbs_email_default_content_type', 'text/html', $this );
		} else if ( ! $this->html ) {
			$this->content_type = 'text/plain';
		}

		return apply_filters( 'kbs_email_content_type', $this->content_type, $this );
	} // get_content_type
	...
	public function send( $to, $subject, $message, $attachments = '' ) {
		...
		$attachments = apply_filters( 'kbs_email_attachments', $attachments, $this );

		$sent       = wp_mail( $to, $subject, $message, $this->get_headers(), $attachments );
		$log_errors = apply_filters( 'kbs_log_email_errors', true, $to, $subject, $message );
	...
	public function text_to_html( $message ) {
		...
		if ( 'text/html' == $this->content_type || true === $this->html ) {
			$message = apply_filters( 'kbs_email_template_wpautop', true ) ? wpautop( $message ) : $message;
		}
